...
public class CompressedMessageTestCase extends QpidJmsTestCase {
    ...
    protected Binary encodeAmqpValueContent(Object content) {
        return encodeContent(new AmqpValue(content));
    }

    protected Binary encodeSerializedJavaContent(String content) throws Exception {
        return encodeContent(new Data(new Binary(getSerializedBytes(content))));
    }

    protected Binary encodeAmqpSequenceContent(List<?> content) {
        return encodeContent(new AmqpSequence(content));
    }

    protected Binary encodeDataContent(byte[] content) {
        return encodeContent(new Data(new Binary(content)));
    }

    protected Binary encodeContent(Section content) {
