...
public abstract class NettyServer implements AutoCloseable {
    ...
    private class NettyServerInboundHandler extends ChannelHandlerAdapter  {
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) {
            ...
            if (msg instanceof WebSocketFrame) {
                ...
                sendHttpResponse(ctx, request, new DefaultFullHttpResponse(HTTP_1_1, BAD_REQUEST, ctx.bufferAllocator().allocate(0)));
            } else {
    ...
    private static void sendHttpResponse(ChannelHandlerContext ctx, FullHttpRequest request, FullHttpResponse response) {
        ...
        if (response.status().code() != 200) {
            Buffer buf = BufferAllocator.onHeapUnpooled().copyOf(response.status().toString().getBytes(StandardCharsets.UTF_8));
