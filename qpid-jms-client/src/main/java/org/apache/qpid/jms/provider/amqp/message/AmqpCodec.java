...
public final class AmqpCodec {
    ...
    private static ReadableBuffer getCachedMessageAnnotationsBuffer(AmqpJmsMessageFacade message, EncoderDecoderContext context) {
        ...
        if (result == null) {
            result = populdateMessageAnnotationsCacheEntry(message, entryKey, context);
        }

        return result.rewind();
    }

    private static ReadableBuffer populdateMessageAnnotationsCacheEntry(AmqpJmsMessageFacade message, Integer entryKey, EncoderDecoderContext context) {
