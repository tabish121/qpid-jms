...
public class IoUringSupport {
    ...
    public static boolean isAvailable(TransportOptions transportOptions) {
        return transportOptions.isUseIoUring() && isAvailable();
    }
    ...
    private static void ensureAvailability() {
        if (!isAvailable()) {
            throw new UnsupportedOperationException(
                "Netty io_uring support is not enabled because the Netty library indicates it is not present or disabled");
        }
    }
}
